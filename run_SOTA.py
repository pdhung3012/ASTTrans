...
fopResultOriginal= fopRepFolder + 'results/originalCS/'
createDirIfNotExist(fopResultOriginal)
...
fpSummary = fopResultOriginal + 'summary.txt'
f1 = open(fpSummary, 'w')
...
for idxProjectName in range(0,len(lstProjectNames)):
    ...
    fopDetailsRank = fopResultOriginal + '{}/details_rank/'.format(currentProjectName)
    fopDetailsTop = fopResultOriginal + '{}/details_top/'.format(currentProjectName)
    fopPredictedRank = fopResultOriginal + '{}/predicted_rank/'.format(currentProjectName)
    fopStoreSortedIndex = fopResultOriginal + '{}/store_sorted_ids/'.format(currentProjectName)
    ...
    for embeddingModel in lstOrgEmbModels:
        ...
        for reductSize in lstOrgDimensionSize:
            try:
                ...
                listKeyQueries = list(dictAllQueries.keys())[:topSelect]
                listKeyCands = list(dictAllCands.keys())[:topSelect]
                listVectorQueries = list(dictAllQueries.values())[:topSelect]
                listVectorCands = list(dictAllCands.values())[:topSelect]
