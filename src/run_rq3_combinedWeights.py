...
lstCombinedWeights=[(w*0.1) for w in range(1,5)]

for cbwValue in lstCombinedWeights:
    fopResultCombine = fopRepFolder + 'results/combineCS_weight_{:.1f}/'.format(cbwValue)
    createDirIfNotExist(fopResultCombine)
    fpConfigFile = fopConfigFolder + 'combinedWeight_{:.1f}_.pkl'.format(cbwValue)
