...
lstDepths=['{}'.format(dim) for dim in range(2,10)]

for dValue in lstDepths:
    fopVectorsAugmented = fopRepFolder + 'data/rq3/depths_vectors_augmented/'
    fopResultCombine = fopRepFolder + 'results/combineCS_depthOfASTTransRep_{}/'.format(dValue)
    createDirIfNotExist(fopResultCombine)
    fpConfigFile = fopConfigFolder + 'depthOfASTTransRep_{}_.pkl'.format(dValue)
    ...
    for idxProjectName in range(0,len(lstProjectNames)):
        ...
        fpAugmentedVectors = '{}/{}/{}.pkl'.format(fopVectorsAugmented, currentProjectName,dValue)
            ...
            for reductSize in lstOrgDimensionSize:
                try:
                    ...
                    if strKeyConfig in dictWeightAugmented.keys():
                        ...
                        # print('com {}'.format(type(combineWeight)))
                    # combineWeight=0
