...
fopResultCombine=fopRepFolder+'results/combineCS_concat/'
createDirIfNotExist(fopResultCombine)
fpConfigFile=fopConfigFolder+'concat.pkl'
            ...
            try:
                ...
                for indexKey in range(0, len(listKeyQueries)):
                    ...
                    if keyQuery in dictTestEmb.keys():
                        ...
                        nl_vecs[indexKey] = nl_vecs[indexKey] + lstAugPred
                        code_vecs[indexKey] = code_vecs[indexKey] + lstAugExp

                    else:
                        itemAugQuery = [0 for i in range(0, augmentedSize)]
                        itemAugCand = [0 for i in range(0, augmentedSize)]
                        nl_vecs[indexKey] = nl_vecs[indexKey] + itemAugQuery
                        code_vecs[indexKey] = code_vecs[indexKey] + itemAugCand
                        print('go here')
